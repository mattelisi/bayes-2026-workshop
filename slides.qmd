---
title: "Bayesian modelling in R and Stan"
subtitle: "<br>_PGR Training 18 Feb 2026_<br>"
author: "Matteo Lisi"
format:
  revealjs:
...
3. Examples:
    ...
    - Multilevel psychometric function with lapses
...
$$p(\underbrace{y}_{\text{data}} \mid \underbrace{\beta_0, \beta_1, \sigma^2}_{\text{parameters}} ) = \underbrace{\prod_{i=1}^{n}}_{\text{product}} \,\, \underbrace{\mathcal{N}(\overbrace{\beta_0 + \beta_1 x_i}^{\text{predicted value}}\,\,,\,\,\, \sigma^2)}_{\text{Gaussian pdf}}$$
...
$$\begin{align} 
\mathcal{L}(y \mid \beta_0, \beta_1, \sigma^2 )  & = \log \left[ L(y \mid \beta_0, \beta_1, \sigma^2)  \right]\\
...
- This is obtained by averaging over the other parameters: $$p(\beta_1 \mid \text{data}) = \iint p(\beta_1, \beta_0, \sigma \mid \text{data}), d\beta_0, d\sigma.$$
...
- **Initialization**:
  ...
  2. choose a probability distribution to generate proposals $g({\bf x}_{n+1}|{\bf x}_n)$ ([_proposal density_]{style="color:#EB641E"})
...
- **For each iteration $i$**:
  ...
  2. calculate the [acceptance ratio]{style="color:#EB641E"} $a = \frac{f({\bf x}')}{f({\bf x}_{i})}$

  3. [accept/reject]{style="color:#EB641E"}: generate a uniform number $u$ in $[0,1]$
...
```{r, fig.align='center', echo=F, fig.width=4, fig.height=4.5, dev='svg', out.width='30%'}
old <- par_dark_slide()
par(mfrow=c(1,1))
...
```{r}
x <- sleepstudy$Days[sleepstudy$Subject=="308"]      # predictor
y <- sleepstudy$Reaction[sleepstudy$Subject=="308"]  # outcome
...
Note: logarithm transform a product into a sum: $\log(x \times y) = \log(x) + \log(y)$, allowing to compute the joint probability of independent events as a sum of log-probabilities.
...
## Bayesian inference after MCMC: it’s just counting {.scrollable}

Once we have posterior samples $\{\beta_1 ^{(1)}, \beta_1 ^{(2)}, \dots, \beta_1 ^{(S)}\}$ any probability statement becomes a simple proportion of samples.

 

::: fragment

**95% credible interval** → Middle 95% of sampled values.

```r
quantile(slope_samples, probs = c(0.025, 0.975))
```
...
**Probability the slope is positive** → $p(\beta_1 > 0 \mid \text{data})$

```r
mean(slope_samples > 0)
```
...
**Probability the slope lies in a meaningful range** → $p(0 < \beta_1 < 5 \mid \text{data})$

```r
mean(slope_samples > 0 & slope_samples < 5)
```
...
**Probability the slope exceeds a threshold** → $p(\beta_1 > 10 \mid \text{data})$

```r
mean(slope_samples > 10)
```
...
## Example 3: Lapse-augmented probit GLMM
 

:::: nonincremental

- Dataset from a [**psychophysics** experiment](https://www.pnas.org/cgi/doi/10.1073/pnas.1901963116) measuring how well people make fine spatial judgements.

- On each trial, two brief (250 ms) peripheral dots appeared; observers pressed Left/Right to report **which dot was farther from fixation**.

- Two viewing conditions varied the dots’ **blur/size & luminance**. **Question:** how does **precision** (steepness of the choice curve) change across conditions?
...
The model at its core is a generalized linear multilevel (GLMM) Probit $$
...
Human observers have imperfect attention, and can sometime press the wrong button my mistake, which can introduce substantial bias in the estimates. To account for this, we extend the model by formulating it as a mixture with a mixture weight $\lambda$

$$P \left(y \mid b\right)  = \frac{1}{2}\lambda \;+ (1 - \lambda) \; \Phi \left( \textbf{X} \beta + \textbf{Z} b \right)$$
:::
...
$\lambda$ is the probability that a participant produce a random response, and should be allowed to vary across participants.

 

::: fragment

We can set $\lambda \sim \text{Beta}(1,7)$ as a prior 
...
plot(x, dbeta(x, 1,7), type="l", col="blue", lwd=2)
...
## Example 3: Lapse-augmented probit GLMM {.scrollable}
